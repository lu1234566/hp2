...
packages. G2 now reads the full name/import/export index of a real HP2 map;
rendering its native BSP geometry is the remaining acceptance criterion.
...
- a metadata-only `hp2_map_probe` targeting `Duel10.unr` for G2a;
...
The gold square follows the left stick. Press **Start** to rescan after an
import. Touch is used only by the launcher, never as gameplay input.
...
G2a additionally generates a metadata-only object index for `Duel10.unr`,
including resolved `Model`, `Polys` and `Level` candidates. The image and all
extracted files are deleted at the end of the run.
...
Actions. Download `HP2-Mobile-G2-dev-debug` from the workflow run's
**Artifacts** section.
...
| G2 | First real HP2 map geometry | IN PROGRESS — package object index implemented |
